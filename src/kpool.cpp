...
inline Parameters parse_args(int& argc, char** argv) {
    ...
    merge->add_option("--tmp-prefix", parameters.tmp_file_prefix, "Prefix for temporary index files (directories need to exist)", true);
    merge->add_option("--pool1", parameters.pool1_id, "Name of the pool for the first table", true);
    merge->add_option("--pool2", parameters.pool2_id, "Name of the pool for the second table", true);

    // Options for 'filter'
    filter->add_option("KMER_TABLE", parameters.table1_file_path, "Path to a merged Jellyfish kmer counts table generated by kpool merge")->required()->check(CLI::ExistingFile);
    filter->add_option("OUTPUT_PREFIX", parameters.output_prefix, "Prefix for output files (filtered kmers for each pool)")->required();
    filter->add_option("--single-pool", parameters.pool_to_filter, "Only output filtered kmers for specified pool", true);
    filter->add_option("--min-presence-depth", parameters.min_kmer_presence_depth, "Minimum depth to consider a kmer present in the focal pool", true);
    filter->add_option("--max-presence-depth", parameters.max_kmer_presence_depth, "Maximum depth to consider a kmer present in the focal pool", true);
    filter->add_option("--max-absence-depth", parameters.max_kmer_absence_depth, "Maximum depth to consider a kmer absent in the other pool", true);
