...
PSASS implements parsing of alignment files using the [htslib](https://github.com/samtools/htslib) library, which depends on zlib, libbz2, and liblzma to read CRAM files. All three libraries are installed by default on most linux distributions.
...
--reference        |  `file`    |  Reference file in fasta format, required with CRAM input files  |         |
--output-file      |  `string`  |  Write output to this file instead of stdout                     |         |
--min-map-quality  |  `string`  |  Minimum mapping quality to include a read in pileup             |    0    |
...
--pool1              |  `string`  |  Name of the first pool                                                 |  females  |
--pool2              |  `string`  |  Name of the second pool                                                |  males    |
--gff-file           |  `string`  |  Path to a GFF file for gene-specific output                            |           |
--popoolation        |            |  If set, assumes the input file was generated with popoolation2         |           |
--snp-file           |  `string`  |  Output pool-specific SNPs to this file                                 |           |
--fst-file           |  `string`  |  Output high FST positions to this file                                 |           |
--min-depth          |  `int`     |  Minimum depth to include a site in the analyses                        |   10      |
--window-size        |  `int`     |  Size of the sliding window (in bp)                                     |   100000  |
--output-resolution  |  `int`     |  Output resolution for sliding window metrics (in bp)                   |   10000   |
--freq-het           |  `float`   |  Allele frequency to consider a SNP heterozygous in a pool              |   0.5     |
--range-het          |  `float`   |  Range of allele frequency to consider a SNP heterozygous in a pool     |   0.15    |
--freq-hom           |  `float`   |  Allele frequency to consider a SNP homozygous in a pool                |   1       |
--range-hom          |  `float`   |  Range of allele frequency to consider a SNP homozygous in a pool       |   0.05    |
--min-fst            |  `float`   |  Minimum FST to output a site in the FST positions file                 |   0.1     |
...
--tmp-prefix       |  `string`  |  Prefix for temporary index files (directories need to exist)    |    ""     |
--pool1            |  `string`  |  Name of the first pool                                          |  females  |
--pool2            |  `string`  |  Name of the second pool                                         |  males    |
--help             |            |  Display help message                                            |           |
...
--single-pool         |  `string`  |  Only output filtered kmers for specified pool                   |           |
--min-presence-depth  |   `int`    |  Minimum depth to consider a kmer present in the focal pool      |       0   |
--max-presence-depth  |   `int`    |  Maximum depth to consider a kmer present in the focal pool      |   99999   |
--max-absence-depth   |   `int`    |  Maximum depth to consider a kmer absent in the other pool       |       0   |
--help                |            |  Display help message                                            |           |
